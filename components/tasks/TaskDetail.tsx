...
function CompactDatePicker({ dueDate, dueTime, reminder, repeat,
  ...
  const CUSTOM_DAY_LABELS = ['S','M','T','W','T','F','S'];
  const [customDays, setCustomDays] = useState<number[]>(() => {
    if (repeat?.startsWith('custom:')) return repeat.replace('custom:','').split(',').map(Number).filter(n=>!isNaN(n));
    return [];
  });
  const remLabel = REMIND_OPTS.find(o=>o.v===reminder)?.l ?? 'None';
  const getRepLabel = (v: string) => {
    if (v === 'none') return 'None';
    if (v === 'weekdays') return 'Weekdays';
    if (v === 'weekends') return 'Weekends';
    if (v.startsWith('custom:')) {
      const days = v.replace('custom:','').split(',').map(Number);
      return days.map(d => CUSTOM_DAY_LABELS[d] ?? '').filter(Boolean).join(' ') || 'Custom';
    }
    if (v === 'custom') return 'Custom';
    return v.charAt(0).toUpperCase()+v.slice(1);
  };
  const repLabel = getRepLabel(repeat);
  const toggleCustomRepeatDay = (day: number) => {
    setCustomDays(prev => {
          ...
          <div className="absolute bottom-full left-0 right-0 z-20 rounded-t-2xl border border-b-0 border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-[#1E2128]">
            ...
            {showRepeat && (
              <div className="py-1">
                ...
                {REPEAT_OPTS.map(o => {
                  const isSelected = o.v === 'custom' ? repeat.startsWith('custom') : repeat === o.v;
                  return (
                    <button key={o.v} type="button" onClick={() => {
                      if (o.v === 'custom') { setRepeat('custom'); }
                      else { setRepeat(o.v); setCustomDays([]); setShowRepeat(false); }
                    }}
                      className={`flex w-full items-center gap-2 py-2 text-sm transition ${o.indent ? 'pl-8 pr-4' : 'px-4'} ${isSelected ? 'text-emerald-600 bg-emerald-50 dark:bg-emerald-900/30' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-white/5'}`}
                    >
                ...
                {repeat.startsWith('custom') && (
                  <div className="px-3 py-2 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div className="flex gap-1">
                      {CUSTOM_DAY_LABELS.map((label, day) => (
                        <button key={day} type="button" onClick={() => toggleCustomRepeatDay(day)}
