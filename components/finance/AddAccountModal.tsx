...
export default function AddAccountModal({ open, onClose, onSave }: AddAccountProps) {
  ...
  const canSubmit = useMemo(() =>
    !!name.trim() && !isNaN(Number(balance)) && (!isCredit || !isNaN(Number(creditLimit))),
  [name, balance, creditLimit, isCredit]);

  const handleSubmit = () => {
    if (!canSubmit) return;
    onSave({ name: name.trim(), type, balance: Number(balance), creditLimit: isCredit ? Number(creditLimit) : undefined });
    setName(''); setBalance('0'); setCreditLimit('0');
    onClose();
  };
    ...
    <Modal open={open} onClose={onClose} title="Add account" subtitle="Link a bank, card, wallet or cash account">
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <select value={type} onChange={e => setType(e.target.value as AccountTypeName)} className={inputCls}>
        ...
        <div>
          ...
          <div className="space-y-3">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700">Current balance</label>
              <input value={balance} onChange={e => setBalance(e.target.value)} placeholder="0" type="number" className={inputCls} />
            </div>
            {isCredit && (
              <div>
                <label className="mb-1.5 block text-sm font-medium text-gray-700">Credit limit</label>
                <input value={creditLimit} onChange={e => setCreditLimit(e.target.value)} placeholder="150000" type="number" className={inputCls} />
              </div>
            )}
          </div>
        </div>
