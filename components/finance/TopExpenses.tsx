  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
      <div className="mb-4 flex items-start justify-between">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Top Expenses · {month}</h2>
      ...
      <div className="space-y-3">
        {rows.map((row, idx) => (
          <div key={row.category} className="flex items-center gap-3">
            ...
            <span className="flex-none text-xs font-semibold text-gray-900">₹{row.amount.toLocaleString('en-IN')}</span>
          </div>
        ))}
      </div>
    </div>
