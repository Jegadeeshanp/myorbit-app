...
import { LayoutDashboard, Landmark, ArrowRightLeft, TrendingUp, CreditCard, Wallet, Activity, Settings } from 'lucide-react';

const MODULE_ICON_BG    = 'linear-gradient(135deg,rgba(0,229,160,0.22),rgba(0,229,160,0.08))';
const MODULE_ICON_BORD  = 'rgba(0,229,160,0.28)';
const MODULE_ICON_GLOW  = '0 0 14px rgba(0,229,160,0.2)';
const MODULE_ICON_COLOR = '#00E5A0';
...
export default function FinanceSidebar() {
  ...
  const active = useMemo(() => {
    if (!pathname) return '/orbit/finance';
    const match = menu.find(item => item.href === pathname);
    if (match) return match.href;
    const prefix = menu.find(item => pathname.startsWith(item.href) && item.href !== '/orbit/finance');
    ...
    <aside className="hidden md:flex" style={{ position: 'sticky', top: 0, height: '100vh', width: 240, flexShrink: 0, background: SB, borderRight: `1px solid ${BORD}`, flexDirection: 'column', overflowY: 'auto' }}>
      ...
      <div style={{ padding: '20px 18px', borderBottom: `1px solid ${BORD}`, display: 'flex', alignItems: 'center', gap: 11 }}>
        <div style={{ width: 36, height: 36, borderRadius: 10, background: MODULE_ICON_BG, border: `1px solid ${MODULE_ICON_BORD}`, boxShadow: MODULE_ICON_GLOW, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <Wallet size={17} color={MODULE_ICON_COLOR} />
        </div>
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: TXT, letterSpacing: '-0.01em' }}>Finance</div>
          <div style={{ fontSize: 9, color: DIM, letterSpacing: '0.14em', textTransform: 'uppercase', marginTop: 1 }}>Money command center</div>
        </div>
      </div>
      ...
      <div style={{ padding: '0 10px 16px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        <Link href="/orbit/finance/settings" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 10, border: '1px solid transparent', color: MUTED, fontSize: 13, textDecoration: 'none' }}>
          <Settings size={15} />
          Settings
        </Link>
