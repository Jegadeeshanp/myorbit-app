...
import { Target, ChevronRight, Calendar, CheckCircle2, Zap } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white py-16 text-center">
          ...
          <p className="font-semibold text-gray-900">No goals yet</p>
          <p className="text-sm text-gray-500 mt-1">Go to Overview to create your first goal</p>
        </div>
      ) : (
        <div className="grid sm:grid-cols-2 gap-4">
          {goals.map(goal => {
              ...
              <div key={goal.id} onClick={() => router.push(`/orbit/goals/${goal.id}`)}
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-2 mb-3">
                    ...
                    {goal.status === 'completed' && <span className="text-xs text-emerald-600 font-medium flex items-center gap-1"><CheckCircle2 className="h-3 w-3" />Done</span>}
                    {goal.status === 'paused' && <span className="text-xs text-amber-600 font-medium">Paused</span>}
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-3">
                      ...
                      {goal.deadline && <span className="text-xs text-gray-500 flex items-center gap-1"><Calendar className="h-3 w-3" />{goal.deadline}</span>}
                    </div>
