...
export default function SignUpPage() {
  ...
  // Client-side Gmail check — server enforces it too
  const emailIsGmail = email.length === 0 || email.toLowerCase().endsWith('@gmail.com');
  ...
  const handleSubmit = async (e: { preventDefault(): void }) => {
    ...
    if (!emailIsGmail) {
      setError('Only Gmail addresses (@gmail.com) are accepted. Use Google sign-up above for other accounts.');
      return;
    }
    ...
    <main className="min-h-screen bg-[#F7F7F5] flex items-center justify-center p-6">
      <div className="w-full max-w-md rounded-3xl bg-white p-9 shadow-lg shadow-emerald-200/40">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">Start tracking your habits, tasks, and life goals.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            {!emailIsGmail && (
              <p className="mt-1.5 text-xs text-red-500">
                Only @gmail.com is accepted. Use &ldquo;Continue with Google&rdquo; above for other accounts.
              </p>
            )}
          </div>
          ...
          <button type="submit"
            disabled={loading || !!oauthLoading || (password.length > 0 && strength.score <= 1) || !emailIsGmail}
